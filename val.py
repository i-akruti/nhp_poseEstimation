...
def convert_to_coco_format(pose_entries, all_keypoints):
    ...
    for n in range(len(pose_entries)):
        ...
        for keypoint_id in pose_entries[n][:-2]:
            ...
            keypoints[to_coco_map[position_id]][0] = cx
            keypoints[to_coco_map[position_id]][1] = cy
...
def infer(net, img, scales, base_height, stride, pad_value=(0, 0, 0), img_mean=(128, 128, 128), img_scale=1/256):
    ...
    avg_heatmaps = np.zeros((height, width, 18), dtype=np.float32)
    avg_pafs = np.zeros((height, width, 30), dtype=np.float32)
...
def evaluate(labels, output_name, images_folder, net, multiscale=False, visualize=False):
    ...
    for sample in dataset:
        ...
        coco_keypoints = convert_to_coco_format(pose_entries, all_keypoints)

        image_id = int(file_name[4:file_name.rfind('.')])
        ...
        if visualize:
            for keypoints in coco_keypoints:
                for idx in range(len(keypoints) // 3):
                    img = cv2.circle(img, (int(keypoints[idx * 3]), int(keypoints[idx * 3 + 1])),
                               20, (255, 0, 0), 2)
            cv2.imwrite(f"../openpose_light_pytorch/val_images/{image_id}.jpg", img)
...
if __name__ == '__main__':
    ...
    class Args():
        def __init__(self):
            self.checkpoint_path = "./Exp2_checkpoints/checkpoint_EPOCH_114.pth"
            self.labels = "../prepared_dataset_monkey/test_pipeline/prepared_val_annotation.json"
            self.images_folder = "../monkey_dataset/test_pipeline/val"
            self.output_name = "detections.json"
    ...
    if args.finetune_monkey:
        net.initial_stage = InitialStage(args.num_channels, args.num_heatmaps, args.num_pafs)
        net.refinement_stages = nn.ModuleList()
        for idx in range(args.num_refinement_stages):
            net.refinement_stages.append(RefinementStage(args.num_channels + args.num_heatmaps + args.num_pafs,
                                                         args.num_channels,
                                                         args.num_heatmaps,
                                                         args.num_pafs))
